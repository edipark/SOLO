...
# -----------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Convert retargeted G1 CSV motion to AMP NPZ")
    parser.add_argument("--csv", required=True, help="CSV with root xyz/xyzw followed by 29 joint positions")
    ...
    default_asset = Path(__file__).parents[1] / "assets"
    parser.add_argument("--urdf", default=str(default_asset / "g1_29dof_rev_1_0.urdf"))
    parser.add_argument("--mesh-dir", default=str(default_asset))
    ...
    if N_orig < 2:
        raise ValueError("Selected frame range must contain at least two frames")
    if joint_data_orig.shape[1] != 29:
        raise ValueError(f"Expected 29 G1 joint columns after the 7 root columns, got {joint_data_orig.shape[1]}")
    ...
    fps_orig = args.input_fps
    dt_orig = 1.0 / fps_orig
    t_orig = np.linspace(0, (N_orig - 1) * dt_orig, N_orig)

    fps_new = args.output_fps
    dt_new = 1.0 / fps_new
    N_new = round((N_orig - 1) * fps_new / fps_orig) + 1
