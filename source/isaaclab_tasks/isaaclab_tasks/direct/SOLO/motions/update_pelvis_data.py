...
def update_pelvis_data(source_file, target_file, output_file=None, dry_run=False):
    """
    ...
    try:
        ...
        # Save to a new path by default; overwriting requires an explicit identical output path.
        output_file = output_file or str(os.path.splitext(target_file)[0] + "_pelvis_fixed.npz")
        np.savez(
            output_file,
            **target_data_dict
        )
        print(f"Successfully wrote updated pelvis data to {output_file}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Copy pelvis trajectory fields between compatible motion NPZ files")
    parser.add_argument("--source", required=True)
    parser.add_argument("--target", required=True)
    parser.add_argument("--output", default=None)
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    update_pelvis_data(args.source, args.target, output_file=args.output, dry_run=args.dry_run)
